...
const emit = defineEmits(['update:page', 'order-click', 'filter-change'])
...
//     const orderNoMatch = filterOrderNo.value
//       ? String(order.uid).includes(filterOrderNo.value)
//       : true
//     const tableIdMatch = filterTableId.value
//       ? String(order.tableId).includes(filterTableId.value)
//       : true
//     return orderNoMatch && tableIdMatch
//   })
// })

// Filtre değişince parent'a haber ver (isteğe bağlı)
// watch([filterOrderNo, filterTableId], () => {
//   emit('filter-change', {
  ...
  >
    ...
    <div class="flex flex-col sm:flex-row gap-2 mb-4">
      <UInput
        ...
        type="text"
        trailing-icon="material-symbols:search"
        placeholder="Sipariş No ile ara"
        class="bg-gray-200 shadow-lg rounded-2xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-200"
      />
      <input
        v-model="filterTableId"
        type="text"
        placeholder="Masa No ile ara"
        class="border rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-200"
      >
    </div>
    ...
    <div v-else class="space-y-3 sm:space-y-4">
      <div
        v-for="(order, index) in props.orders" :key="order.uid"
    ...
    <!-- Pagination kartın içinde ve en altta olmalı -->

    <!-- <UPagination
      :page="props.page"
      :total="props.totalOrders"
      :page-count="Math.ceil(props.totalOrders / props.pageSize)"
      :page-size="props.pageSize"
      :ui="{ root: '!items-center !justify-center !mx-auto' }"
      class="mt-4 !items-center !justify-center !mx-auto !z-50"
